...
const unsigned char timetable[5][5] =
{
  {162, 167, 167, 127, 128},
  {226, 60, 60, 0, 0},
  {225, 60, 59, 0, 0},
...
pos48274:
  ...
  A = *(sampleTable + mem47*256+Y);       // sampleTable[mem47*256+Y];
pos48280:
...
// pos47587:
  do
  {
    ...
    phase1 = *(tab47492 + stressOutput[Y] + 1);     // tab47492[stressOutput[Y] + 1];
...
//   241     0     6     0    73     0    99    61
  ...
  while (1)      //while No. 1
  {
    ...
    X = A;
    mem56 = *(blendRank+A);             //blendRank[A];
    A = *(blendRank+Y);             //blendRank[Y];

    // compare the rank - lower rank value is stronger
    if (A == mem56)
    {
      // same rank, so use out blend lengths from each phoneme
      phase1 = *(outBlendLength+Y);                  //outBlendLength[Y];
      phase2 = *(outBlendLength+X);                  //outBlendLength[X];
    } else
    if (A < mem56)
    {
      // first phoneme is stronger, so us it's blend lengths
      phase1 = *(inBlendLength+X);                  //inBlendLength[X];
      phase2 = *(outBlendLength+X);                  //outBlendLength[X];
    } else
    {
      ...
      phase1 = *(outBlendLength+Y);                  //outBlendLength[Y];
      phase2 = *(inBlendLength+Y);                  //inBlendLength[Y];
    }
...
//
  ...
  for (i = 255; i >= 0; i--)
  {
    amplitude1[i] = *(amplitudeRescale + amplitude1[i]);            //amplitudeRescale[amplitude1[i]];
    amplitude2[i] = *(amplitudeRescale + amplitude2[i]);            //amplitudeRescale[amplitude2[i]];
    amplitude3[i] = *(amplitudeRescale + amplitude3[i]);            //amplitudeRescale[amplitude3[i]];
  }
